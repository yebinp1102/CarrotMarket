import {
  useQuery,  // use when fetching data
  useMutation,  // use when update data
  useQueryClient,
  useInfiniteQuery
...
export const useDeleteSavedPost = () => {
 ...
 // userId, name과 같은 유저 기본 정보를 포함해서 어떤 post를 like하고 save 했는지에 대한 정보도 제공하는 쿼리 
...
export const useSearchPosts = (searchTerm: string) => {
  return useQuery({
    queryKey: [QUERY_KEYS.SEARCH_POSTS, searchTerm], // validate query again, when searchTerm changes
    queryFn: () => searchPosts(searchTerm),
    enabled: !!searchTerm // when searchTerm changes, useSearchPosts query will be automatically fetched.
  })
}